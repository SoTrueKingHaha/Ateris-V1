...
namespace WinAPI
{
	...
	__inline void* GetProcessAddr(DWORD dwModule, const char* szProcess) {
		...
		if ((char*)pAddress >= (char*)pExpDir && (char*)pAddress < (char*)pExpDir + pExpEntry->Size) {
			...
			*func_name++ = 0;
			...
			DWORD dwGetModuleHandle = (DWORD)GetProcessAddr(GetKernel32Addr(), ("GetModuleHandleA"));
			...
			if (!frwd_module) {
				DWORD dwLoadLibrary = (DWORD)GetProcessAddr(GetKernel32Addr(), ("LoadLibraryA"));
	...
	__inline void* CallAPI(const char* pszModule, const char* pszFunction, int arguments, ...) {
		HANDLE hModule;
	...
	__inline void FreeLibraryAndExitThread(HMODULE hLibModule, DWORD dwExitCode) {
		CallAPI(KERNEL, ("FreeLibraryAndExitThread"), 2, dwExitCode, hLibModule);
	}
	...
	__inline void GetPrivateProfileString(LPCSTR lpAppName, LPCSTR lpKeyName, LPCSTR lpDefault, LPSTR lpReturnedString, DWORD nSize, LPCSTR lpFileName) {
		CallAPI(KERNEL, ("GetPrivateProfileStringA"), 6, lpFileName, nSize, lpReturnedString, lpDefault, lpKeyName, lpAppName);
	}

	__inline void WritePrivateProfileString(LPCSTR lpAppName, LPCSTR lpKeyName, LPCSTR lpString, LPCSTR lpFileName) {
		CallAPI(KERNEL, ("WritePrivateProfileStringA"), 4, lpFileName, lpString, lpKeyName, lpAppName);
	}

	__inline BOOL CloseHandle(HANDLE hObject) {
		return reinterpret_cast<BOOL>(CallAPI(KERNEL, ("CloseHandle"), 1, hObject));
	}

	__inline BOOL PathFileExists(LPCSTR pszPath) {
		return reinterpret_cast<BOOL>(CallAPI(("Shlwapi.dll"), ("PathFileExistsA"), 1, pszPath));
	}

	__inline BOOL GetUserName(LPTSTR lpBuffer, LPDWORD lpnSize) {
		return reinterpret_cast<BOOL>(CallAPI(("Advapi32.dll"), ("GetUserNameA"), 2, &lpnSize, lpBuffer));
	}

	__inline HMODULE GetModuleHandleW(LPCWSTR lpModuleName) {
		return reinterpret_cast<HMODULE>(CallAPI(KERNEL, ("GetModuleHandleW"), 1, lpModuleName));
	}
	...
	__inline HANDLE CreateThread(LPSECURITY_ATTRIBUTES lpThreadAttributes, SIZE_T dwStackSize, LPTHREAD_START_ROUTINE lpStartAddress, LPVOID lpParameter, DWORD dwCreationFlags, LPDWORD lpThreadId) {
		return reinterpret_cast<HANDLE>(CallAPI(KERNEL, ("CreateThread"), 6, lpThreadId, dwCreationFlags, lpParameter, lpStartAddress, dwStackSize, lpThreadAttributes));
	}
	...
	__inline HANDLE CreateRemoteThread(HANDLE hProcess, LPSECURITY_ATTRIBUTES lpThreadAttributes, SIZE_T dwStackSize, LPTHREAD_START_ROUTINE lpStartAddress, LPVOID lpParameter, DWORD dwCreationFlags, LPDWORD lpThreadId) {
		return reinterpret_cast<HANDLE>(CallAPI(KERNEL, ("CreateRemoteThread"), 7, lpThreadId, dwCreationFlags, lpParameter, lpStartAddress, dwStackSize, lpThreadAttributes, hProcess));
	}

	__inline HWND FindWindowW(LPCWSTR lpClassName, LPCWSTR lpWindowName) {
		return reinterpret_cast<HWND>(CallAPI(USER32, ("FindWindowW"), 2, lpWindowName, lpClassName));
	}

	__inline HWND GetForegroundWindow() {
		return reinterpret_cast<HWND>(CallAPI(USER32, ("GetForegroundWindow"), 0));
	}

	__inline DWORD GetLastError() {
		return reinterpret_cast<DWORD>(CallAPI(KERNEL, ("GetLastError"), 0));
	}

	__inline int MessageBoxA(HWND hWnd, char* lpText, char* lpCaption, UINT uType) {
		return reinterpret_cast<int>(CallAPI(USER32, ("MessageBoxA"), 4, uType, lpCaption, lpText, hWnd));
	}

	__inline int MessageBoxW(HWND hWnd, LPCWSTR lpText, LPCWSTR lpCaption, UINT uType) {
		return reinterpret_cast<int>(CallAPI(USER32, ("MessageBoxW"), 4, uType, lpCaption, lpText, hWnd));
	}
}