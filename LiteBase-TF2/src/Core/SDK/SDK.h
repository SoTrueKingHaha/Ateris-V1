...
#include "Include/TF2/iclientrenderable.h"
#include "Include/TF2/icliententity.h"
#include "Include/TF2/icliententitylist.h"
#include "Include/TF2/inputsystem.h"
#include "Include/TF2/gamemovement.h"
#include "Include/TF2/imovehelper.h"
#include "Include/TF2/IEngineTrace.h"
#include "Include/TF2/globalinfo.h"
#include "Include/TF2/Panel.h"
#include "Include/TF2/soundparam.h"
#include "Include/TF2/viewrender.h"
#include "Include/TF2/ccaptureflag.h"
#include "Include/TF2/tfplayeranimstate.h"
#include "Include/EntityCache/EntityCache.h"
#include "../Features/Vars.h"
#include "../Utils/WinAPI/WinAPI.h"
#include "../Utils/DirectX/DirectX.h"
#include "Include/TF2/MD5.h"
...
enum DataCenter_t
{
	...
	//EU East		- Frankfurt, Stockholm, Warsaw, Vienna			- 6, 25, 29, 28
	//EU West		- Amsterdam, London, Luxembourg, Madrid, Paris	- 0, 13, 15, 17, 21
	//Asia			- Manila, Singapore								- 18, 24
	//Middle East	- Dubai											- 3
	//Hong Kong		- Hong Kong										- 9
	//Japan			- Tokyo (GNRT)									- 7, 27
	//India			- Mumbai, Chennai								- 2, 16
	//Australia		- Sydney										- 26
	//Africa		- Johannesburg									- 11
	//Chile			- Santiago										- 22
	//Peru			- Lima											- 14
	//Brazil		- Sao Paulo										- 8
	//US East		- Atlanta, Virginia, Chicago					- 1, 10, 20
	//US North West - Moses Lake, Washington, Seattle				- 4, 5, 23
	//US South West - Los Angeles, Oklahoma City					- 12, 19
};
...
namespace Utils
{
	...
	inline Cond GetCondByID( ETFCond id )
	{
		/*if ( id == TF_COND_FEIGN_DEATH || id == TF_COND_STEALTHED_BLINK || id == TF_COND_STEALTHED )
		{
			return { L"CLOAKED", { 200, 200, 200, 255 } };
		}

		if ( id == TF_COND_INVULNERABLE_WEARINGOFF || id == TF_COND_INVULNERABLE_USER_BUFF || id == TF_COND_INVULNERABLE_CARD_EFFECT || id == TF_COND_INVULNERABLE )
		{
			return { L"UBERCHARGE", { 255, 110, 255, 255 } };
		}

		if ( id == TF_COND_CRITBOOSTED_PUMPKIN || id == TF_COND_CRITBOOSTED_USER_BUFF || id == TF_COND_CRITBOOSTED_DEMO_CHARGE || id == TF_COND_CRITBOOSTED_FIRST_BLOOD ||
			 id == TF_COND_CRITBOOSTED_BONUS_TIME || id == TF_COND_CRITBOOSTED_CTF_CAPTURE || id == TF_COND_CRITBOOSTED_ON_KILL || id == TF_COND_CRITBOOSTED_RAGE_BUFF ||
			 id == TF_COND_CRITBOOSTED_CARD_EFFECT || id == TF_COND_CRITBOOSTED_RUNE_TEMP || id == TF_COND_CRITBOOSTED )
		{
			return { L"CRIT BOOST", { 255, 150, 255, 255 } };
		}*/
	...
	inline const wchar_t* GetWeaponNameByType( CTFWeaponBase* pWeapon )
	{
	...
	inline IMaterial* CreateMaterial(const char* szVars)
	{
		static int nCreatedMats = 0;
		char szOut[512];
		sprintf_s(szOut, sizeof(szOut), ("material%i.vmt"), nCreatedMats++);

		char szMaterial[512];
		sprintf_s(szMaterial, sizeof(szMaterial), szVars);

		KeyValues* pVals = new KeyValues(("matvalsxd"));
		g_KeyValUtils.Initialize(pVals, (char*)szOut);
		g_KeyValUtils.LoadFromBuffer(pVals, szOut, szMaterial);

		IMaterial* pCreated = I::MaterialSystem->CreateMaterial(szOut, pVals);
	...
	inline const char* GetClassByIndex(const int nClass)
	{
	...
	inline void GetProjectileFireSetup(CTFPlayer* pPlayer, const Vec3& vViewAngles, Vec3 vOffset, Vec3* vSrc)
	{
		if (g_ConVars.cl_flipviewmodels->GetInt())
			vOffset.y *= -1.0f;

		Vec3 vecForward = Vec3(), vecRight = Vec3(), vecUp = Vec3();
		Utils::AngleVectors(vViewAngles, &vecForward, &vecRight, &vecUp);
	...
	inline bool VisPos(const Vec3 &from, const Vec3 &to)
	{
		...
		Trace(from, to, (MASK_SHOT | CONTENTS_GRATE), &filter, &trace); //MASK_SOLID
		return trace.fraction > 0.99f;
	}

	inline bool VisPos(CBaseEntity* pSkip, CBaseEntity * pEntity, const Vec3& from, const Vec3& to)
	{
		CGameTrace trace = {};
		CTraceFilterHitscan filter = {};
		filter.pSkip = pSkip;
	...
	inline bool VisPosMask( CBaseEntity *pSkip, const CBaseEntity *pEntity, const Vec3 &from, const Vec3 &to, unsigned int nMask )
	{
		CGameTrace trace = {};
		CTraceFilterHitscan filter = {};
		filter.pSkip = pSkip;
	...
	inline bool VisPosHitboxId(CBaseEntity* pSkip, CBaseEntity * pEntity, const Vec3& from, const Vec3& to, int nHitbox)
	{
		CGameTrace trace = {};
		CTraceFilterHitscan filter = {};
		filter.pSkip = pSkip;
	...
	inline bool VisPosHitboxIdOut(CBaseEntity* pSkip, CBaseEntity * pEntity, const Vec3& from, const Vec3& to, int& nHitboxOut)
	{
		CGameTrace trace = {};
		CTraceFilterHitscan filter = {};
		filter.pSkip = pSkip;
	...
	inline bool VisPosFraction(CBaseEntity* pSkip, const Vec3& from, const Vec3& to)
	{
		CGameTrace trace = {};
		CTraceFilterHitscan filter = {};
		filter.pSkip = pSkip;
	...
	inline bool WillProjectileHit( CTFPlayer *pLocal, CTFWeaponBase *pWeapon, const Vec3 &targetPos )
	{
		CGameTrace trace{};
		...
		const Vec3 aimAngles = Utils::CalcAngle( shootPos, targetPos );
...
#pragma warning (disable: 4244)
	...
	inline void FixMovement(CUserCmd* pCmd, const Vec3& vecTargetAngle)
	{
		Vec3 vecMove(pCmd->forwardmove, pCmd->sidemove, pCmd->upmove);
		Vec3 vecMoveAng = Vec3();

		Utils::VectorAngles(vecMove, vecMoveAng);

		float fSpeed = Utils::FastSqrt(vecMove.x * vecMove.x + vecMove.y * vecMove.y);
	...
	inline float ATTRIB_HOOK_FLOAT( float base_value, const char *search_string, CBaseEntity *ent, void *buffer, bool is_global_const_string )
	{
	...
	inline Color_t GetHealthColor(int nHP, int nMAXHP)
	{
		nHP = std::max(0, std::min(nHP, nMAXHP));
		int r = std::min(((510 * (nMAXHP - nHP)) / nMAXHP), 255);
		int g = std::min(((510 * nHP) / nMAXHP), 255);
		return { static_cast<byte>(r), static_cast<byte>(g), 75, 255 };
	}

	inline Color_t GetIndicatorColor( int nCurrent, int nMax, unsigned char b )
	{
		nCurrent = std::max( 0, std::min( nCurrent, nMax ) );
		int r = std::min( ( ( 510 * ( nMax - nCurrent ) ) / nMax ), 200 );
		int g = std::min( ( ( 510 * nCurrent ) / nMax ), 200 );
	...
	inline Color_t GetKDColor(float nKD, float nMAXKD)
	{
		nKD = std::max(0.f, (std::min(nKD, nMAXKD)));
		float r = std::min((510 * (nMAXKD - nKD)) / (nMAXKD), 255.f);
		float g = std::min((510 * nKD) / (nMAXKD) + 50, 255.f);
		return { static_cast<byte>(r), static_cast<byte>(g), 0, 255 };
	}

	__inline Vec3 GetWorldSpaceCenter(CBaseEntity* pEntity) {
		Vec3 vMin, vMax;
		pEntity->GetRenderBounds(vMin, vMax);
		Vec3 vWorldSpaceCenter = pEntity->GetAbsOrigin();
		vWorldSpaceCenter.z += (vMin.z + vMax.z) / 2.0f;
		return vWorldSpaceCenter;
	}

	inline EWeaponType GetWeaponType(CTFWeaponBase* pWeapon)
	{
		if (!pWeapon)
	...
	inline bool GetDrawBounds(CBaseEntity* pEntity, Vec3* vTrans, int& x, int& y, int& w, int& h)
	{
		Vec3 vMins, vMaxs;
		...
		for (int n = 0; n < 8; n++)
			VectorTransform(vPoints[n], transform, vTrans[n]);
			...
			&& W2S(vTrans[6], blt) && W2S(vTrans[7], flt))
		{
			...
			return !(x > g_Draw.m_nScreenW || x + w < 0 || y > g_Draw.m_nScreenH || y + h < 0);
		}
	...
	{
		Vec3 WSC = GetWorldSpaceCenter(pEntity);

		if (WSC.DistTo(GetWorldSpaceCenter(pLocal)) > 300.0f)
		{
			Vec3 vScreen = {};

			if (Utils::W2S(GetWorldSpaceCenter(pEntity), vScreen))
			{
				if (vScreen.x < -400
					|| vScreen.x > g_Draw.m_nScreenW + 400
					|| vScreen.y < -400
					|| vScreen.y > g_Draw.m_nScreenH + 400)
					return false;
			}
	...
	inline Vector ComputeMove( const CUserCmd *pCmd, CTFPlayer *pLocal, Vec3 &a, Vec3 &b )
	{
		const Vec3 diff = ( b - a );
		if ( diff.Length( ) == 0.0f )
		{
		...
		Vec3 ang;
		Utils::VectorAngles( vSilent, ang );
	...
	inline void WalkTo( CUserCmd *pCmd, CTFPlayer *pLocal, Vec3 &a, Vec3 &b, float scale )
	{
	...
	inline void WalkTo( CUserCmd *pCmd, CTFPlayer *pLocal, Vec3 &pDestination )
	{
		Vec3 localPos = pLocal->m_vecOrigin( );
		WalkTo( pCmd, pLocal, localPos, pDestination, 1.f );
	}

	inline void StopMovement( CUserCmd *pCmd, bool safe = true ) {
		if ( safe && g_GlobalInfo.m_bAttacking ) { return; }

		if ( CTFPlayer *pLocal = GET_ENT_I( LOCAL_IDX )->As<CTFPlayer>( ) ) {
			const float direction = Utils::VelocityToAngles( pLocal->m_vecVelocity( ) ).y;
			...
			g_GlobalInfo.m_bShouldStop = false;
		}
	}

	inline bool IsAttacking( const CUserCmd *pCmd, CTFWeaponBase *pWeapon )
	{
...
namespace G
{
	...
	inline std::wstring GetBuildingName( CBaseEntity *pEntity )
	{
		switch ( pEntity->GetClassId( ) )
		{
			case EClassIds::CObjectSentrygun:
			{
				const auto &pSentry = pEntity->As<CObjectSentrygun>( );
	...
	inline std::wstring GetWorldName( CBaseEntity *pAnimated )
	{
		const auto &pModel = pAnimated->As<CBaseEntity>( );
		if ( Hash::IsHealth( pModel->GetModelName( ) ) )
		{
			return L"Healthpack";
		}

		if ( Hash::IsAmmo( pModel->GetModelName( ) ) || pAnimated->GetClassId( ) == EClassIds::CTFAmmoPack)
		{
			return L"Ammopack";
		}

		switch ( pAnimated->GetClassId( ) )
		{
			...
			{
				const auto &pProjectile = pAnimated->As<CBaseProjectile>( );
				if ( pProjectile->As<CTFGrenadePipebombProjectile>( )->m_iType( ) == 1 )
				{
					return L"Sticky";
				}
				else
			...
			case EClassIds::CTFProjectile_ThrowableRepel:
			{
				return L"Jarate";
			}
	...
	inline std::wstring GetNPCName( CBaseEntity *pNpc )
	{
		switch ( pNpc->GetClassId( ) )
		{
			case EClassIds::CHeadlessHatman:
			{
				return L"Headless Horsemann";
				break;
			}
